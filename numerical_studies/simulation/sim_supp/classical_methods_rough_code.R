...
## E. Hartman, R. Grieve, R. Ramsahai, and J. S. Sekhon. From sample average treatment effect to popu-
#lation average treatment effect on the treated: combining experimental with observational studies to
#estimate population treatment effects. Journal of the Royal Statistical Society Series A: Statistics in
#Society, 178(3):757–778, 2015.

# Perform genetic matching
m.out <- matchit(
  A ~ X1 + X2 + X3 + X4 + I(3*X5),
  data = dat.t,
  method = "genetic",
...
#View(matched.data[order(matched.data$subclass),])
...
#####################################################################
## S. R. Cole and E. A. Stuart. Generalizing evidence from randomized clinical trials to target populations:
#the actg 320 trial. American journal of epidemiology, 172(1):107–115, 2010.
...
# Combine datasets
dat.all <- rbind(dat.t[, c("X1", "X2", "X3", "X4", "X5", "in_rct")],
                 dat.os[, c("X1", "X2", "X3", "X4", "X5", "in_rct")])

# Fit random forest to predict inclusion
rf_inclusion <- randomForest(as.factor(in_rct) ~ X1 + X2 + X3 + X4 + X5,
                             data = dat.all,
                             ntree = 500,
                             importance = TRUE)
...
# Predict on RCT covariates (X1 to X5)
X.rct <- dat.t[, c("X1", "X2", "X3", "X4", "X5")]
...
entire_nums_wgts = pmin(treatment_probs_os*(1-inclusion_probs)/inclusion_probs, 10)
