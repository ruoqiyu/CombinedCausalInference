...
rct_pval <- function(beta0, data, rblock=NULL, delta=0, ri=FALSE, nsim=10^6){
  ...
  if(!ri)
    return(t.test(Ywt~Z, data=data, mu=-(beta0+delta), alternative='less')$p.val)
  ...
  return(as.numeric(summary(res)[3]))
}

foo_rct <- function(beta0, data, rblock=NULL, delta=0, alpha=0.05, ri=FALSE, nsim=10^6){	
...
os_pval <- function(beta0, ymat, gamma=1){
  ...
  #wrong: ymat_extprbeta0[,1] - rowMeans(ymat_extprbeta0[,-1,drop=FALSE])
  extmu = mean(extauvec)
  extvr = var(tauvec - extauvec)
  zvalbeta0 = (tauhat - extmu - beta0)/sqrt(extvr/nrow(ymat))
  #print(c(mean(extprbeta0$maxmu), extmu, extvr, zvalbeta0))
  pnorm(zvalbeta0, lower=FALSE)
}


foo_os <- function(beta0, ymat, gamma=1, alpha=0.05){
  os_pval(beta0,ymat=ymat, gamma) - alpha
}
...
foo_combined <- function(beta0, ymat, gamma=1, data, rblock=NULL, delta=0, ri=FALSE, nsim=10^6, alpha=0.05){
  -2*log(os_pval(beta0,ymat=ymat, gamma=gamma)) -
    2*log(rct_pval(beta0,data=data, rblock=rblock, delta=delta, ri=ri, nsim=nsim)) -  
    qchisq(1-alpha, 4)
}
